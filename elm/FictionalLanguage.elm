module FictionalLanguage
  exposing(engToHexa, visualToHexa, hexaToEng, hexaToVisual, hexaToPhonetic, FieldModel, hexaToListFieldModel)
{-| FromEnglish
...
import String exposing (join, split, lines, words, cons, contains, startsWith, dropLeft, foldr, trim)
...
toEng s = map wordToEng s

toListFieldModel: List(Word) -> List(FieldModel)
toListFieldModel s = map wordToFieldModel s
...
wordList = lines csvContent |> List.map csvToTuple

specialWordList: List (String, String)
specialWordList = lines csvSpecialWords |> List.map csvToTuple

swapWordList =  map (\t -> (second(t), first(t))) wordList

wordDict = Dict.fromList (wordList)
swapWordDict = Dict.fromList (swapWordList)
specialWordDict = Dict.fromList (specialWordList)

wordToHexa: Word -> String
wordToHexa s =
  case s of
  X w -> toHexa(w) |> join "C"
  C w -> toHexa(w) |> join "D" --2nd level ?
  N n -> cons 'A' n
  Unknown -> unknownHexa
  W w -> getOrDefaultDict w wordDict

wordToEng: Word -> String
wordToEng s =
  case s of
  X w -> toEng(w) |> join "/"
  C w -> toEng(w) |> join ":" --2nd level ?
  N n -> cons '#' n
  Unknown -> unknownHexa
  W w -> w
